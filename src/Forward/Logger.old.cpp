...
Logger::Logger() {
    infoStream.open("log/info.txt", std::ofstream::out | std::ofstream::binary);
    warnStream.open("log/warn.txt", std::ofstream::out | std::ofstream::binary);
    errorStream.open("log/warn.txt", std::ofstream::out | std::ofstream::binary);
}
...
void Logger::output(ofstream& ofs, const string& id, const string& message) {
    ofs << "\n[" << id << "] " << message << " " << Util::timeToStr() << endl;
}
