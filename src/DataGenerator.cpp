...
const static string sqlTemplate_car_signal = "select signal_value, build_time from car_signal where carid=(?) and signal_type=(?) and build_time<(?) order by build_time desc limit 1";
const static string sqlTemplate_car_signal1 = "select signal_value, build_time from car_signal1 where carid=(?) and signal_type=(?) and build_time<(?) order by build_time desc limit 1";
const static string sqlTemplate_car_trace_gps_info = "select latitude, longitude from car_trace_gps_info where carid=(?) and build_time<(?) order by build_time desc limit 1";
...
void DataGenerator::generateDataTaskB() {
    try {
        ...
        // task 1: 补发之前错过的周期数据，从数据库取出数据生成ByteBuffer直接丢入队列
        for (m_currUploadTime += m_staticResource->Period; m_currUploadTime < periodTaskBeginTime; m_currUploadTime += m_staticResource->Period) {
            ...
            currUploadTimeTM = localtime(&m_currUploadTime);
            memset(strCurrUploadTime, 0, sizeof (strCurrUploadTime));
            strftime(strCurrUploadTime, 19, TIMEFORMAT, currUploadTimeTM);
            m_prepStmtForSig->setDateTime(3, strCurrUploadTime);
            m_prepStmtForBigSig->setDateTime(3, strCurrUploadTime);
            m_prepStmtForGps->setDateTime(2, strCurrUploadTime);
            createCarListDataFromDB(true, true);
        }

        currUploadTimeTM = localtime(&periodTaskBeginTime);
        memset(strCurrUploadTime, 0, sizeof (strCurrUploadTime));
        strftime(strCurrUploadTime, 19, TIMEFORMAT, currUploadTimeTM);
        m_prepStmtForSig->setDateTime(3, strCurrUploadTime);
        m_prepStmtForBigSig->setDateTime(3, strCurrUploadTime);
        ...
        //task4: 当周期时间点到达时，遍历map，将最近一个周期内没有上传的车机数据丢入队列，最近一个周期内已上传过的车机就不重复上传。
        for (;;) {
